...
const targetCircles = circles.filter(c => targetIds.includes(c.id) || ['10', '11', '15', '16'].includes(String(c.num)));

console.log("Target circles found:", targetCircles.map(c => ({ id: c.id, num: c.num, cx: c.cx, cy: c.cy })));

targetCircles.forEach(c => {
  console.log(`\n--- Circle (${c.cx}, ${c.cy}) num=${c.num} ---`);
  let minDist = Infinity;
  let bestShape = null;
  lotShapes.forEach((s, idx) => {
  ...
  console.log(`Closest shape distance: ${minDist.toFixed(2)} units. Centroid: (${bestShape?.centroid.x.toFixed(1)}, ${bestShape?.centroid.y.toFixed(1)})`);
});
